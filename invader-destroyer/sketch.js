...
let boostpower = -10;
let boostCooldown = 5000; // boost cooldown in milliseconds
let lastBoostTime = 0;
...
let damageCooldown = 500; // cooldown for invincibility frames after taking damage
let lastDamageTime = 0; 
let heartdropCooldown = 60000; // cooldown for heart drop every minute
let lastHeartDropTime = 0;
let enemyspawnCooldown = 150; // cooldown between spawning one enemy and another 
...
function preload() {
  ...
  playerDamage = loadSound('playerDamage.mp3');
  playerBoost = loadSound('playerBoost.mp3');
  heartPickup = loadSound('heartPickup.mp3');
  heart = loadImage('red-heart.png');
}
...
function displayUI() {
  ...
  fill(255, 0, 0);
  rect(20, height - 80, 100 * healthRatio, 20);
  noFill();
  stroke(255);
  rect(20, height - 80, 100, 20);
...
function moveObjects() {
  ...
  x = constrain(x, playerSize/2 + 250 , width - playerSize/2 - 250); 
...
function displayObjects() {
  ...
  // display heart falling from the top of the screen 
  if (millis() - lastHeartDropTime > heartdropCooldown) { 
    let heartObj = {
      x: random(250, width - 250),
      y: -50
    };
    hearts.push(heartObj);
    lastHeartDropTime = millis();
  }
  for (let i = hearts.length - 1; i >= 0; i--) {
    let heartObj = hearts[i];
    heartObj.y += 2;
    image(heart, heartObj.x - 15, heartObj.y - 15, 30, 30);
    if (heartObj.y > height) {
      hearts.splice(i, 1);
    }
  }
  ...
  if (enemiesSpawned < enemyAmount) {
    if (millis() - lastEnemySpawnTime > enemyspawnCooldown) {
...
function checkCollisions() {
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    let enemy = enemies[i];
    let d = dist(enemy.x, enemy.y, x, y);
  ...
  // Check for collision between player and hearts and increase score if player picks up heart when at full health
  for (let i = hearts.length - 1; i >= 0; i--) {
    let heartObj = hearts[i];
    let d = dist(heartObj.x, heartObj.y, x, y); 
    if (d < 15 + playerSize/2) {
...
function healthloss() {
  // decrease player health by 1 and check for game over and cooldown for invincibility frames to prevent instant death from multiple collisions
